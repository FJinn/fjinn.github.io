...
public:
	int myIndex;
	Vector myPosition, initialVelocity;
	Vector ahead;
...
*/
	...
	void DrawBoid(Attractor* att, Obstacle* obs[], Repeller* rep)
	{
		//==========================================================================
		...
		float theta;
		theta = atan2(initialVelocity.mVal[1], initialVelocity.mVal[0]);
	...
	void Seek(Attractor* att)
	{
		// target position
		Vector target(att->myPosition.mVal[0], att->myPosition.mVal[1], 0);

		Vector desiredVelocity(target.mVal[0] - myPosition.mVal[0], target.mVal[1] - myPosition.mVal[1], 0) ;
		...
		Vector steeringForce(desiredVelocity.mVal[0] - initialVelocity.mVal[0], desiredVelocity.mVal[1] - initialVelocity.mVal[1], 0);
		steeringForce.normalize();
		float i = maxSteering / steeringForce.length();
		i = i < 0.05 ? i : 0.05;
		steeringForce *= i;

		initialVelocity += steeringForce;

		i = maxVelocity / initialVelocity.length();
		i = i < 1.0 ? i : 1.0;
		initialVelocity *= i;
	}
	...
	{
		ahead.set(initialVelocity.mVal[0], initialVelocity.mVal[1], 0);
		ahead.normalize();
		ahead = myPosition + ahead;

		for (int n = 0; n < totalObstacle; n++)
		{
			float distance = sqrt(pow(ahead.mVal[0] - obs[n]->myPosition.mVal[0], 2.0) + pow(ahead.mVal[1] - obs[n]->myPosition.mVal[1], 2.0));
			if (distance < obs[n]->myRadius - radiusFix)
			{
				Vector avoidForce(ahead.mVal[0] - obs[n]->myPosition.mVal[0], ahead.mVal[1] - obs[n]->myPosition.mVal[1], 0);
				avoidForce.normalize();
				avoidForce *= obs[n]->myRadius - distance;

				float i = maxAvoid / avoidForce.length();
				i = i < 0.7 ? i : 0.7;
				avoidForce *= i;

				initialVelocity += avoidForce;
	...
	void AvoidRepeller(Repeller *rep)
	{
		ahead.set(initialVelocity.mVal[0], initialVelocity.mVal[1], 0);
		ahead.normalize();
		ahead *= maxVelocity;
		...
		float distance = sqrt(pow(ahead.mVal[0] - rep->myPosition.mVal[0], 2.0) + pow(ahead.mVal[1] - rep->myPosition.mVal[1], 2.0));
		if (distance < rep->myRadius - radiusFix)
		{
			Vector repelForce(ahead.mVal[0] - rep->myPosition.mVal[0],ahead.mVal[1] - rep->myPosition.mVal[1], 0);
...
*/
	...
	for (int i=0; i< totalBoids; i++)
	{
		...
		{
			Vector temp;
			temp.set(boid[i]->myPosition.mVal[0] - boid->myPosition.mVal[0], boid[i]->myPosition.mVal[1] - boid->myPosition.mVal[1], boid[i]->myPosition.mVal[2] - boid->myPosition.mVal[2]);
...
Vector BoidRuleThree(Boid* boid)
{
	...
	*/
	...
	for (int i = 0; i < totalBoids; i++)
	{
		if (i != boid->myIndex)
		{
			...
			if (temp.length() > 0 && temp.length() < 50)
			{
				pvJ = pvJ + boid[i]->initialVelocity;
				neighbourCount++;
			}
		}
	}
...
void Display(GLFWwindow* window)
{
	...
	for (int i = 0; i < totalBoids; i++)
	{
		boid[i]->initialVelocity += BoidRuleOne(boid[i]) + BoidRuleTwo(boid[i]) + BoidRuleThree(boid[i]);

		float j = maxVelocity / boid[i]->initialVelocity.length();
		j = j < 1.0 ? j : 1.0;
		boid[i]->initialVelocity *= j;

		boid[i]->DrawBoid(att, obs, rep);

		j = maxVelocity / boid[i]->initialVelocity.length();
		j = j < 1.0 ? j : 1.0;
		boid[i]->initialVelocity *= j;
	}
