...
private:
	...
	GLfloat mRectTexture[RECT_VERTEX_COUNT * 2];
...
public:
	void init()
	{
		...
		loadTexture("../media/water.bmp", mTextureID[0]);
	...
	void genPlane(float offsetRadian)
	{
		...
		for (int h = 0; h<RECT_VERTICE_H - 1; h++)
		{
			for (int w = 0; w<RECT_VERTICE_W - 1; w++)
			{
				...
				mRectTexture[vertex1 * 2 + 1] = (float)(h) / (float)RECT_VERTICE_H * texMul;
				...
				mRectTexture[vertex2 * 2 + 1] = (float)(h) / (float)RECT_VERTICE_H * texMul;
				...
				mRectTexture[vertex3 * 2 + 1] = (float)(h + 1) / (float)RECT_VERTICE_H * texMul;
				...
				mRectTexture[vertex5 * 2 + 1] = (float)(h + 1) / (float)RECT_VERTICE_H * texMul;
